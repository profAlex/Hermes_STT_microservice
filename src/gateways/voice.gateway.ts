    ...
    private async processAudioPipeline(ws: WebSocket, audioStream: PassThrough): Promise<void> {
        try {
            while (ws.readyState === WebSocket.OPEN && !audioStream.destroyed) {
                ...
                // Во время работы Whisper аудио от сокета продолжают идти в PassThrough.
                // Применяем resume(), чтобы очищать буфер от "шума" во время обработки Whisper.
                audioStream.resume();
